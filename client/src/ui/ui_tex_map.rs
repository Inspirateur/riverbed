use bevy::{
    asset::LoadedFolder, color::palettes::css, image::TRANSPARENT_IMAGE_HANDLE, prelude::*,
};
...
use crate::render::{
    parse_block_tex_name, BlockTexState, BlockTextureFolder, ItemTexState, ItemTextureFolder,
};
...
impl Plugin for UiTexMapPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(UiTextureMapState::default())
...
impl UiTextureMap {
    ...
    pub fn make_item_slot(&self, node: &mut ChildSpawnerCommands, stack: &Stack, kind: UiSlotKind) {
        let alpha = if kind == UiSlotKind::Disabled {
            0.4
        } else {
            1.
        };
        node.spawn((
            ImageNode {
                ...
                color: match stack {
                    Stack::Some(Item::Block(block), _) if block.is_foliage() => {
                        Color::linear_rgba(0.3, 1.0, 0.1, alpha)
                    }
            ...
            BackgroundColor(
                if kind == UiSlotKind::Disabled || kind == UiSlotKind::NoBg {
                    Color::NONE
                } else {
...
fn load_ui_item_textures(
    ...
    for item_handle in item_folder.handles.iter() {
        ...
        ui_tex_map
            .0
            .insert(item, item_handle.clone().try_typed().unwrap());
    }
...
fn load_ui_block_textures(
    ...
    for block_handle in block_folder.handles.iter() {
        ...
        tex_map.insert(
            (block, face_specifier),
            block_handle.clone().try_typed().unwrap(),
        );
    }
