...
impl Earth {
    pub fn sample_col(&self, x: i32, z: i32) -> (Vec<f32>, Vec<f32>, Vec<f32>) {
        let [y, t, h] = self.sample( x * CHUNK_S1,
            z * CHUNK_S1,
            CHUNK_S1 as usize,
            CHUNK_S1 as usize,
            SCALE,
...
impl NoiseFct<3> for Earth {
    fn build(&self, n: &mut Noise<f64>) -> [f32; 3] {
        let land = n.noise(1.) + n.noise(3.) * 0.3 + n.noise(9.) * 0.1;
        let land = land.mask(self.landratio);
        let mount_mask = n.noise(2.).mask(0.2);
        let mount = (n.noise(4.).abs() + n.noise(8.).abs() * 0.3).norm() * land.clone() * mount_mask;
        let y = (land + mount).norm();
        let t = (1. - y.clone()*0.5) * (n.noise(0.6)*0.5+0.5);
        let h = (1. - (t.clone() - 0.7).pow(2) * 2.) * (n.noise(1.)*0.5+0.5);
