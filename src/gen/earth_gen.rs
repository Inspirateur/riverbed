...
pub const WATER_H: i32 = (crate::blocs::MAX_GEN_HEIGHT as f32*WATER_R) as i32;
...
impl TerrainGen for Earth {
    fn gen(&self, world: &mut Blocs, col: ColPos) {
        ...
        let landratio = self.config.get("land_ratio").copied().unwrap_or(0.5);
        let cont = (n.simplex(0.1) + n.simplex(0.5) * 0.4).normalize();
        let land = (cont.clone() + n.simplex(3.)*0.2 + n.simplex(9.)*0.1).normalize().threshold(landratio);
        let ocean = !cont.pos();
