...
use super::{ui_tex_map::UiTextureMap, ControllingPlayer, ItemHolder, UIAction, UISlot};
...
impl Plugin for HotbarPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(Update, refresh_hotbar_items)
...
fn setup_hotbar_display(
    ...
    for i in 0..HOTBAR_SLOTS {
        ...
        commands.spawn(NodeBundle { style: img_style, ..Default::default()})
            ...
            .with_children(|node|
                UiTextureMap::make_empty_item_slot(node)
            );
    }
}

fn refresh_hotbar_items(
    node_query: Query<(&HotbarSlot, &Children)>,
    ...
    for (slot, children) in node_query.iter() {
        let selected = slot.0 == selected_slot.0;
        for child in children {
            if let Ok(mut ui_img) = img_query.get_mut(*child) {
                *ui_img = tex_map.get_with_alpha(&hotbar[slot.0], if selected { 1. } else { 0.5 });
            }
            if let Ok(mut text) = text_query.get_mut(*child) {
                let quantity = hotbar[slot.0].quantity();
...
fn scroll_hotbar(
    ...
    for (slot, children) in node_query.iter() {
        ...
        for child in children {
            if let Ok(mut ui_img) = img_query.get_mut(*child) {
                ui_img.color.set_alpha(if selected { 1. } else { 0.5 });
            }
