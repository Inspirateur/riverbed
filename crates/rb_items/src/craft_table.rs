...
pub fn parse_recipes(recipes: &str) -> Vec<CraftEntry> {
    ...
    recipes
        .trim()
        .lines()
        .map(|recipe| match expander.try_expand(recipe) {
            None => CraftEntry::Recipe(parse_recipe(recipe)),
            Some(recipe_group) => {
                CraftEntry::RecipeGroup(recipe_group.into_iter().map(parse_recipe).collect())
            }
        })
        .collect_vec()
}
...
mod tests {
    ...
    fn test_parsing() {
        ...
        assert_eq!(
            group1[0],
            Recipe {
                ingredients: vec![(Item::Block(Block::AcaciaLog), 1), (Item::Rock, 4)],
